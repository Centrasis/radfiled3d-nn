...
// a D3D12 resource is the COMPUTE side's knowledge — it owns the external-memory API that does the
...
bool ExternalMemory::supports(Backend compute) const noexcept {
    if (!nn::dx12::available()) return false;
...
std::shared_ptr<memory::MemoryRef> ExternalMemory::import_buffer(const ExternalBuffer& buffer,
    ...
    if (!nn::dx12::available())
        throw Exception::feature_disabled("dx12", "importing a D3D12 resource as an inference target");
    if (!backend.available())
        throw Exception::feature_disabled(backend.get_feature(), "importing a D3D12 resource as an inference target");
...
ExternalBuffer ExternalMemory::export_buffer(std::uint64_t, Backend compute) {
    ...
    if (!backend.can_import_from(get_domain()))
        throw Exception::unsupported_interop(to_string(get_domain()), to_string(compute));
    if (!nn::dx12::available())
        throw Exception::feature_disabled("dx12", "exporting a device buffer to D3D12");
    throw Exception::feature_disabled(backend.get_feature(), "exporting a device buffer to D3D12");
}
